...
% Graph nodes
% -----------------------------
\node[main] (v) at (0,0) {$v$\\[-1mm]\scriptsize $h_v^{(t-1)}$};

\node[neigh] (u1) at (-2.2,1.8) {$u_1$\\[-1mm]\scriptsize $h_{u_1}^{(t-1)}$};
\node[neigh] (u2) at (-2.4,-1.8) {$u_2$\\[-1mm]\scriptsize $h_{u_2}^{(t-1)}$};
\node[neigh] (u3) at (2.2,1.8) {$u_3$\\[-1mm]\scriptsize $h_{u_3}^{(t-1)}$};
...
% Arrows showing flow
\draw[->, very thick, blue!70] (u1) -- (-0.55,0.45);
\draw[->, very thick, blue!70] (u2) -- (-0.55,-0.45);
\draw[->, very thick, blue!70] (u3) -- (0.55,0.45);

% Neighborhood label
\node[lab] at (0,-2.9) {$N_G(v)=\{u_1,u_2,u_3\}$};

% -----------------------------
% Aggregation block
% -----------------------------
\node[box] (agg) at (5.8,0.8) {
$\displaystyle
m_v^{(t)} = \mathscr{A}\bigl(\{\,h_u^{(t-1)} : u \in N_G(v)\,\}\bigr)
$
};

\draw[->, very thick] (1.0,0) -- node[above, lab] {representaciones vecinas} (4.2,0.8);

% -----------------------------
% Update block
% -----------------------------
\node[box] (upd) at (11.5,0.8) {
$\displaystyle
h_v^{(t)} = \mathscr{U}\!\left(h_v^{(t-1)},\, m_v^{(t)}\right)
$
};

\draw[->, very thick] (7.5,0.8) -- (9.9,0.8);

% Updated node
\node[main, fill=green!18] (newv) at (15.5,0.8) {$v$\\[-1mm]\scriptsize $h_v^{(t)}$};

\draw[->, very thick] (13.3,0.8) -- (14.7,0.8);

% -----------------------------
% Final embedding annotation
% -----------------------------
\node[smallbox] (final) at (11.5,-2.2) {
$\displaystyle z_u = h_u^{(K)}$
};

\node[lab] at (11.5,-3.2) {Representación final del nodo tras $K$ iteraciones};

% -----------------------------
% Step annotation
% -----------------------------
\node[lab, anchor=west] at (-3.4,3.0) {\textbf{Iteración \(t\):} propagación local en el grafo};
\node[lab, anchor=west] at (4.0,2.3) {\textbf{1.} Agregación de vecinos};
\node[lab, anchor=west] at (9.8,2.3) {\textbf{2.} Actualización del estado};
