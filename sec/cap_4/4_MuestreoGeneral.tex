...
En los últimos años, el Aprendizaje Profundo (\textit{Deep Learning}) se ha consolidado sobre bases matemáticas sólidas procedentes del álgebra lineal, el análisis matemático y la estadística \cite{Goodfellow-et-al-2016}. Sin embargo, la interacción entre ambas disciplinas no se limita a la fundamentación teórica de los modelos; por el contrario, las redes neuronales se emplean cada vez más como herramientas exploratorias para analizar la estructura de objetos matemáticos abstractos y complejos \cite{bronstein2021geometric}.

En este contexto, el presente capítulo aborda el estudio y modelado de superficies tridimensionales construidas a partir de nudos de Lissajous, definidos mediante funciones armónicas periódicas que dan lugar a variedades con diversas configuraciones topológicas y grados de autointersección. Si bien las Redes Neuronales de Gráficas (\textit{Graph Neural Networks}, GNNs) \cite{xu2019powerful} han demostrado ser efectivas en el dominio de datos no euclidianos, las implementaciones convencionales presentan limitaciones para reconocer invariantes topológicos debido a que el muestreo euclidiano tradicional pierde la conectividad y adyacencia intrínsecas de la variedad \cite{2025eulearn}.

Con el fin de superar esta limitación, el objetivo principal de este capítulo es desarrollar la metodología para la generación de estas superficies e implementar un algoritmo de muestreo adaptativo no euclidiano. Tal como proponen 
\cite{2025eulearn}, este esquema de muestreo preserva la estructura topológica y la información de adyacencia, optimizando el entrenamiento y la capacidad predictiva de las redes de aprendizaje profundo. A lo largo del capítulo se detallará la metodología empleada, la cual integra el procesamiento geométrico, la representación gráfica, el algoritmo de muestreo diseñado y la evaluación de las GNNs en tareas de clasificación topológica, resaltando el potencial de estas herramientas para abordar problemas fundamentales en la teoría de nudos y la geometría computacional.

\section{Construcción de superficies a partir de símplices}
...
Un archivo en formato STL describe este tipo de superficies poligonales en $\mathbb{R}^3$ mediante una colección finita de triángulos. A partir de esta representación, se construye una estructura combinatoria asociada asignando un nodo a cada vértice geométrico y añadiendo, por cada triángulo, las tres aristas que unen sus vértices. Este procedimiento, implementado mediante la librería \textbf{networkX}, produce una gráfica que corresponde al $1$-esqueleto de la triangulación de la superficie.
En este contexto, la malla triangulada induce de manera natural un complejo simplicial bidimensional $K = (V,E,F)$, donde $V$ es el conjunto de vértices, $E$ el conjunto de aristas y $F$ el conjunto de caras triangulares (véase la definición formal en el Capítulo 1). La gráfica construida mediante \textbf{networkX} representa entonces el $1$-esqueleto de dicho complejo.
No obstante, para estudiar rigurosamente el objeto como una superficie, no basta considerar únicamente la gráfica; es necesario incorporar también la información de las caras triangulares. La realización geométrica del complejo simplicial $K$ proporciona una aproximación discreta de la superficie original y permite analizar propiedades topológicas globales como la conectividad, la presencia de frontera y la característica de Euler,
\[
\chi = |V| - |E| + |F|.
\]
...
\section{Transformación superficie a gráfica}
...
\[
\operatorname{pos}(v) = (x_v,y_v,z_v)\in \mathbb{R}^3,
\]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\textwidth]{img/grafica_superficie.png}
    \caption{Representación tridimensional del $1$-esqueleto de la superficie triangulada asociada a una curva de Lissajous engrosada. Los nodos, mostrados en azul, corresponden a los vértices de la malla, mientras que las aristas representan la conectividad entre vértices adyacentes en el complejo simplicial. La estructura resultante define una gráfica no dirigida embebida en $\mathbb{R}^3$, que preserva la geometría global de la superficie original. Esta visualización permite analizar la distribución espacial de los nodos, así como las relaciones de vecindad y conectividad inherentes a la discretización.}
    \label{fig:superficie_grafica}
\end{figure}
...
\begin{tikzpicture}[
    scale=0.5,
    transform shape,
    shift={(-9,0)}, % ajusta este valor
    every node/.style={font=\small},
    original node/.style={
        circle, draw=black!50, thick,
        fill=cyan!35, minimum size=9mm, inner sep=0pt
    },
    mst node/.style={
        circle, draw=black!50, thick,
        fill=green!35, minimum size=9mm, inner sep=0pt
    },
    edge label/.style={
        fill=white, inner sep=1pt, font=\small
    }
]

\node[font=\Large\bfseries] at (3.8,6.7) {Gráfica original};
\node[font=\Large\bfseries] at (15.2,6.7) {Árbol Generador Mínimo};


% Nodos
\node[original node] (o0) at (0,2.2) {0};
\node[original node] (o1) at (2.2,5.2) {1};
\node[original node] (o2) at (2.2,2.2) {2};
\node[original node] (o3) at (2.2,-0.8) {3};

\node[original node] (o4) at (5.0,5.2) {4};
\node[original node] (o5) at (5.0,2.2) {5};
\node[original node] (o6) at (5.0,-0.8) {6};

\node[original node] (o7) at (7.2,2.2) {7};

% Aristas
\draw[gray!70, thick] (o1) -- (o4)
    node[midway, above, edge label] {0};

\draw[gray!70, thick] (o0) -- (o2)
    node[midway, above, edge label] {1};

\draw[gray!70, thick] (o2) -- (o5)
    node[midway, above, edge label] {8};

\draw[gray!70, thick] (o5) -- (o7)
    node[midway, above, edge label] {9};

\draw[gray!70, thick] (o3) -- (o6)
    node[midway, below, edge label] {7};

\draw[gray!70, thick] (o1) -- (o2)
    node[midway, right, edge label] {3};

\draw[gray!70, thick] (o2) -- (o3)
    node[midway, right, edge label] {2};

\draw[gray!70, thick] (o4) -- (o5)
    node[midway, right, edge label] {1};

\draw[gray!70, thick] (o5) -- (o6)
    node[midway, right, edge label] {6};

\draw[gray!70, thick] (o0) -- (o1)
    node[midway, above left, edge label, rotate=55] {10};

\draw[gray!70, thick] (o0) -- (o3)
    node[midway, below left, edge label, rotate=-55] {4};

\draw[gray!70, thick] (o3) -- (o5)
    node[midway, below right, edge label, rotate=45] {2};

\draw[gray!70, thick] (o4) -- (o7)
    node[midway, above right, edge label, rotate=-55] {8};

\draw[gray!70, thick] (o6) -- (o7)
    node[midway, below right, edge label, rotate=55] {12};

% Nodos
\node[mst node] (m0) at (10.2,2.1) {0};
\node[mst node] (m2) at (12.7,2.1) {2};
\node[mst node] (m3) at (12.7,-0.8) {3};

\node[mst node] (m1) at (12.4,5.2) {1};
\node[mst node] (m4) at (15.3,5.2) {4};
\node[mst node] (m5) at (15.3,2.1) {5};
\node[mst node] (m6) at (15.3,-0.8) {6};
\node[mst node] (m7) at (17.8,2.1) {7};

% Aristas MST
\draw[blue, thick] (m1) -- (m4)
    node[midway, above, edge label] {0};

\draw[blue, thick] (m4) -- (m5)
    node[midway, right, edge label] {1};

\draw[blue, thick] (m5) -- (m6)
    node[midway, right, edge label] {6};

\draw[blue, thick] (m4) -- (m7)
    node[midway, above right, edge label, rotate=-55] {8};
...
\draw[blue, thick] (m3) -- (m5)
    node[midway, above left, edge label, rotate=45] {2};

    \end{tikzpicture}

\caption{Comparación entre la gráfica original y el árbol generador mínimo obtenido.}
\label{fig:original_mst}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{img/flujo.png}
    \caption{Flujo del algoritmo de muestro por esferas.}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Muestreo.png}
        \caption{Muestreo uniforme en la esfera.}
        \label{fig:muestreo_uniforme}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Muestreo2.png}
        \caption{Grafo de vecindad esférica.}
        \label{fig:grafo_vecindad}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Muestreo3.png}
        \caption{Vecindad local.}
        \label{fig:vecindad_local}
    \end{subfigure}
    \hfill
    \begin{subfigure}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Muestreo4.png}
        \caption{Resumen del algoritmo.}
        \label{fig:resumen_algoritmo}
    \end{subfigure}

    \caption{Proceso de muestreo uniforme y construcción de la gráfica de vecindad esférica.}
    \label{fig:muestreo_esfera}
\end{figure}
\newpage
\section{Reasignación de adyacencias}
...
\begin{figure}[H]
    ...
    \caption{Flujo del algoritmo de reducción de nodos.}
    \label{fig:flujo algoritmo}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.95\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Adyacency_1.png}
        \caption{Etapas 1 a 3 del algoritmo.}
        \label{fig:algoritmo_parte1}
    \end{subfigure}
    ...
    \begin{subfigure}[t]{0.95\textwidth}
        \centering
        \includegraphics[width=\textwidth]{img/Adyacency_2.png}
        \caption{Etapas 4 a 6 del algoritmo.}
        \label{fig:algoritmo_parte2}
    \end{subfigure}

    \caption{Proceso de eliminación de un nodo y redistribución de sus conexiones.}
    \label{fig:algoritmo_eliminacion}
\end{figure}
\section{Método general para la reducción de nodos}
