...
A partir de las definiciones previas y del Teorema de Radó, es posible introducir nuevos objetos que servirán como base para la caracterización formal de las superficies de interés. De este modo, estableceremos una relación entre superficies y gráficas que facilitará el estudio y análisis eficiente de las superficies consideradas en este trabajo.
\begin{definition}[Encaje]
    Sea $A \subset X$ un espacio topológico y $e : A \rightarrow X$ una aplicación continua.
    A $e$ se le llama encaje si la restricción $e':A \rightarrow e(A)$, tal que $e'(a) = e(a)$ es un homeomorfismo al dotar a $e(A)$ con la topología relativa.
\end{definition}

\begin{definition}[Arco simple]
    Sea $X$ un espacio topológico y $\alpha$ un encaje.
    Definimos un \textbf{arco simple} en $X$ como la imagen de un encaje $\alpha: I \rightarrow X$, donde $I = [0,1]$ es el intervalo unitario. Es decir, un arco simple es una trayectoria continua e inyectiva en $X$, cuya imagen no se cruza a sí misma.
\end{definition}
...
Regularmente denotamos a $V$ y $E$ como $V(G)$ y $E(G)$ que hacen referencia a la gráfica G, entonces $G=(V(G),E(G))$
Definimos a $E$ como:
\begin{center}
$E(V)$ = $\{\{u,v\} | u,v \in V, u \neq v \}$
\end{center}
