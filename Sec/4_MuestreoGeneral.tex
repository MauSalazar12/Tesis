...
\Function{Sampling}{$x, adjacency, size, alcance, esfera\_size$}
    ...
    \State Inicializar $removed\_nodes[] \gets empty\_list$, $queue[] = order\_copy$
...
El bucle principal del algoritmo, que se ejecuta hasta alcanzar el tamaño deseado de la muestra, domina la mayor parte del costo computacional, en el peor de los casos este bucle se ejecuta \(V\) veces ya que en cada iteración se elimina un nodo por tanto su costo podría acotarse por \(\mathcal{O} (V)\). 
...
Luego, la complejidad de algoritmo para generar la muestra uniforme está determinado por el número de veces que se ejecuta su bucle principal, ya que, las operaciones realizadas no dependen del número de puntos a generar por tanto su complejidad es $\mathcal{O}(1)$, de esta forma la complejidad de este algortimo es $\mathcal{O}(esfera_size)$, que para terminos de la implementación de dicho algoritmo, siempre es constante. 
...
Finalmente, los nodos eliminados deben ser descartados de la matriz de adyacencia, lo que implica un costo adicional de \( \mathcal{O}(V \cdot k) \), donde \( k \) es el número de nodos eliminados que para terminos de la implementación lo podemos dejar como una constante, por lo tanto la complejidad final sería $\mathcal{O}(V)$.
