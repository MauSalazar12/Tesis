...
\Function{Sampling}{$x, adjacency, size, alcance, esfera\_size$}
    ...
    \State Inicializar $visitados[] \gets FALSE$ , $orden[] \gets []$
    \State Obtener orden mediante DFS $dfs\_order(0, visitados, orden) $\Comment{Algoritmo \ref{alg:DFS}} 

    \State Inicializar $nodos\_eliminados[] \gets []$, $cola[] = orden[]$
    
    \While{$numero\_nodos - nodos\_eliminados \geq size$}
        \If{$nodos\_eliminados$ no es vacío}
            \State $dist = ||nodo\_actual - centroide||_2 $
            \State Ordena $cola$ por distancias en orden descendente
        \EndIf
        \State Obtener coordenadas del primer elemento de $cola$.
        \\ \hfil $idx = cola.pop(0)$ $h, k, l = x[idx]$ 
        \State Obtenemos esfera uniforme \Call{GeneraPuntosUniformes}{$idx$} \Comment{Algoritmo \ref{alg:generaPuntos}}
        \State Hacemos $vecinos = vecinos \cap GeneraPuntosUniformes(idx)$
        \ForAll{$vecinos \in vecinos \setminus eliminados$}
