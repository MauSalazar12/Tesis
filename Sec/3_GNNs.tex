...
% =======================
% Neuronas de la capa L-1 (entrada)
% =======================
\node[neuron] (x1) {$x^{(L-1)}_1$};
\node[neuron, below=0.8cm of x1] (x2) {$x^{(L-1)}_2$};
\node at ($(x2) - (0,1.2)$) {$\vdots$};
\node[neuron, below=2.4cm of x2] (xn) {$x^{(L-1)}_{n_{L-1}}$};

% Caja de entrada
\node[draw, thick, fit=(x1) (x2) (xn), inner sep=12pt] (inputbox) {};

% =======================
% Bloque lineal (W y b)
% =======================
\node[draw, rectangle, thick, right=2.5cm of inputbox, minimum width=2cm, minimum height=5cm, align=center] (linear) {$\mathbf{W}^{(L)}$ \\ $+$ \\ $\mathbf{b}^{(L)}$};

% =======================
% Bloque de activación sigma
% =======================
\node[draw, rectangle, thick, right=2.5cm of linear, minimum width=1.5cm, minimum height=5cm, align=center] (sigma) {$\sigma$};

% =======================
% Neuronas de salida (capa L)
% =======================
\node[neuron, right=2.5cm of sigma] (y1) {$f^{(L)}_1$};
\node[neuron, below=0.8cm of y1] (y2) {$f^{(L)}_2$};
\node at ($(y2) - (0,1.2)$) {$\vdots$};
\node[neuron, below=2.4cm of y2] (yn) {$f^{(L)}_{n_L}$};

% Caja de salida
\node[draw, thick, fit=(y1) (y2) (yn), inner sep=12pt] (outputbox) {};

% =======================
% Conexiones
% =======================
% Entradas hacia bloque lineal
\foreach \i in {x1,x2,xn}
    \draw[arrow] (\i.east) -- (linear.west);

% Lineal hacia sigma
\draw[arrow] (linear.east) -- (sigma.west);

% Sigma hacia salidas
\foreach \j in {y1,y2,yn}
    \draw[arrow] (sigma.east) -- (\j.west);
